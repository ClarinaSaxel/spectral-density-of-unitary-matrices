...
\begin{equation} \label{eq:real_symmetric_inner_product}
    \lambda v^* v = v^* (\lambda v) = v^* (A v) = (A^* v)^* v = (A v)^* v = (\lambda v)^* v = \overline{\lambda} v^* v \\
\end{equation}
...
The complex equivalent of a symmetric matrix is a \emph{Hermitian matrix}:
...
\begin{equation} \label{eq:unitary_eigenvalue_equation}
